...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, vocab, hps):
    ...
    self.abs_ids = [vocab.word2id(w) for w in abstract_words] # list of word ids; OOVs are represented by the id for UNK token

    # Get the decoder input sequence and target sequence
    self.dec_input, self.target = self.get_dec_inp_targ_seqs(self.abs_ids, hps.max_dec_steps, start_decoding, stop_decoding)
    ...
    if hps.pointer_gen:
      ...
      self.abs_ids_extend_vocab = data.abstract2ids(abstract_words, vocab, self.article_oovs)

      # Overwrite decoder target sequence so it uses the temp article OOV ids
      _, self.target = self.get_dec_inp_targ_seqs(self.abs_ids_extend_vocab, hps.max_dec_steps, start_decoding, stop_decoding)
...
class Batch(object):
  ...
  def __init__(self, example_list, hps, vocab):
    ...
    self.start_decoding = vocab.word2id(data.START_DECODING)
    self.stop_decoding = vocab.word2id(data.STOP_DECODING)
  ...
  def init_decoder_seq(self, example_list, hps):
    ...
    # use for adding bidirectional decoder
    self.back_dec_batch = np.zeros((hps.batch_size, hps.max_dec_steps), dtype=np.int32)
    self.target_batch = np.zeros((hps.batch_size, hps.max_dec_steps), dtype=np.int32)
    self.back_target_batch = np.zeros((hps.batch_size, hps.back_max_dec_steps), dtype=np.int32)
    self.dec_padding_mask = np.zeros((hps.batch_size, hps.max_dec_steps), dtype=np.float32)

    # Fill in the numpy arrays
    for i, ex in enumerate(example_list):
      self.dec_batch[i, :] = ex.dec_input[:]
      temp = ex.dec_input[1:ex.dec_len]
      self.back_dec_batch[i][0] = self.stop_decoding
      self.back_dec_batch[i][1:ex.dec_len] = temp[::-1]
      self.back_dec_batch[i][ex.dec_len:] = 1
      self.target_batch[i, :] = ex.target[:]
      temp = ex.target[:ex.target_len-1]
      self.back_target_batch[i][:ex.target_len-1] = temp[::-1]
      self.back_target_batch[i][ex.target_len-1] = self.start_decoding
      self.back_target_batch[i][ex.target_len:] = 1
  ...
  def store_orig_strings(self, example_list):
    ...
    self.abs_ids_extend_vocab = [ex.abs_ids_extend_vocab for ex in example_list] # list of list
    self.abs_ids = [ex.abs_ids for ex in example_list] # list of list
